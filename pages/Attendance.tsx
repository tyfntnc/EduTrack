...
export const Attendance: React.FC<AttendanceProps> = ({ currentUser, onCourseClick }) => {
  ...
  // Branş ikonlarını eşleştiren yardımcı fonksiyon
  const getBranchIcon = (branchId: string) => {
    switch (branchId) {
      case 'b1': return '⚽'; // Futbol
      case 'b2': return '🏀'; // Basketbol
      case 'b3': return '📐'; // Matematik
      case 'b4': return '🏐'; // Voleybol
      case 'b5': return '🏊'; // Yüzme
      default: return '📚';
    }
  };
    ...
    return (
      <div className="flex items-center justify-center gap-2 mt-4 pb-2">
        <button 
          ...
          className="p-2 rounded-xl border border-slate-100 bg-white disabled:opacity-30 active:scale-90 transition-all shadow-sm"
        >
  ...
  return (
    <div className="w-full page-transition px-4 space-y-6 pt-4">
      {/* Search - Top Positioned */}
      <div className="relative">
        ...
        <input 
          ...
          className="w-full bg-slate-50 border border-slate-100 rounded-2xl pl-10 pr-4 py-3.5 text-xs font-medium focus:ring-2 focus:ring-indigo-500/20 transition-all outline-none"
        />
      </div>
      ...
      <div className="space-y-8 pb-10">
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between px-1">
            ...
            {totalTakenPages > 1 && (
              <span className="text-[8px] font-bold text-indigo-400 bg-indigo-50 px-2 py-0.5 rounded-full uppercase">
          ...
          <div className="space-y-2.5">
            {paginatedTaken.length === 0 ? (
              <div className="py-10 text-center bg-slate-50 rounded-[2rem] border border-dashed border-slate-200">
                 <p className="text-[10px] font-bold text-slate-300 uppercase">Kayıtlı ders bulunamadı</p>
              </div>
            ) : (
              paginatedTaken.map(c => (
                <button 
                  ...
                  className="w-full text-left p-3.5 bg-white border border-slate-100 rounded-[1.75rem] flex items-center gap-4 shadow-sm active:scale-[0.98] transition-all"
                >
                  <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-xl shadow-inner border border-slate-100 shrink-0">
            ...
            <div className="space-y-2.5">
              {paginatedGiven.map(c => (
                <button 
                  key={c.id} 
                  onClick={() => onCourseClick?.(c.id)}
                  className="w-full text-left p-3.5 bg-indigo-50/50 border border-indigo-100 rounded-[1.75rem] flex items-center gap-4 active:scale-[0.98] transition-all"
                >
                  <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-xl shadow-lg shadow-indigo-100 shrink-0 border border-indigo-500/20">
                    {getBranchIcon(c.branchId)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-bold text-indigo-900 truncate">{c.title}</h4>
                    <p className="text-[10px] font-bold text-indigo-600/60 mt-0.5 uppercase tracking-tighter">
