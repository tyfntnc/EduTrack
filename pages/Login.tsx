...
export const Login: React.FC<LoginProps> = ({ onLogin, onRegisterClick, onForgotClick, onBackToLanding }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    // Yapay gecikme ile giriş simülasyonu
    setTimeout(() => {
      setIsLoading(false);
      onLogin(email);
    }, 1500);
  ...
  return (
    <div className="fixed inset-0 z-[400] bg-white dark:bg-slate-950 flex flex-col p-8 transition-colors duration-500 overflow-y-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest ml-1">Şifre</label>
          <div className="relative">
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300">
            ...
            <input 
              ...
              placeholder="••••••••"
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl pl-12 pr-12 py-4 text-sm font-bold text-slate-900 dark:text-slate-100 outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all"
            />
            <button 
        ...
        <div className="flex items-center justify-between px-1">
          <label className="flex items-center gap-2 cursor-pointer group">
            <input type="checkbox" className="w-4 h-4 rounded border-slate-200 dark:border-slate-800 text-indigo-600 focus:ring-indigo-500/20" />
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Beni Hatırla</span>
          </label>
          <button type="button" onClick={onForgotClick} className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">Şifremi Unuttum</button>
        </div>
      ...
      <div className="mt-10 space-y-6">
        <div className="relative flex items-center justify-center">
          ...
          <span className="relative bg-white dark:bg-slate-950 px-4 text-[9px] font-black text-slate-300 dark:text-slate-700 uppercase tracking-[0.2em]">Veya Şununla Bağlan</span>
        </div>

        <div className="flex gap-4">
          <button className="flex-1 py-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-all">
            <img src="https://www.gstatic.com/images/branding/product/1x/gsa_512dp.png" className="w-5 h-5" alt="Google" />
            <span className="text-[10px] font-black uppercase tracking-widest dark:text-slate-300">Google</span>
          </button>
          <button className="flex-1 py-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-all">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest dark:text-slate-300">Facebook</span>
          </button>
        </div>
      </div>
      ...
      <footer className="mt-auto py-8 text-center">
        <p className="text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest">
          Hesabın yok mu? <button onClick={onRegisterClick} className="text-indigo-600 dark:text-indigo-400 font-black">Hemen Kayıt Ol</button>
        </p>
      </footer>
    </div>
  );
