...
export const Dashboard: React.FC<DashboardProps> = ({ userRole, userName, currentUserId }) => {
  ...
  const QRModal = () => (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 page-transition">
      ...
      <div className="bg-white w-full max-w-[320px] rounded-[2.5rem] p-8 relative z-10 shadow-2xl flex flex-col items-center animate-in zoom-in-95 duration-300">
        <h3 className="text-xl font-bold text-slate-900 mb-1">Giriş QR Kodu</h3>
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6 text-center">Yoklama için bu kodu okutun</p>
  ...
  return (
    <div className="w-full page-transition px-4 space-y-5 overflow-x-hidden">
      {isQRModalOpen && <QRModal />}
      
      {/* Hero Header */}
      <section className="pt-4 flex justify-between items-center">
        <div className="space-y-0.5">
          <p className="text-indigo-600 text-[10px] font-bold uppercase tracking-widest leading-none">Hoş Geldin</p>
          <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight leading-none">{userName.split(' ')[0]} 👋</h2>
        </div>
        <div className="text-right">
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Bugünkü Derslerin</p>
          <p className="text-lg font-black text-indigo-600 leading-none">{todayClasses.length}</p>
        </div>
      </section>

      {/* Primary Action Card (Performance + QR) */}
      <section className="bg-slate-900 rounded-[2.25rem] p-5 text-white relative overflow-hidden shadow-xl shadow-slate-200">
        <div className="relative z-10 flex justify-between items-center">
          <div className="space-y-4 flex-1">
            <div>
              <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-1">Performans Özeti</p>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sıradaki Dersler</h3>
          <button className="text-[10px] font-bold text-indigo-600 active:opacity-50">Tümünü Gör</button>
        </div>
        
        <div className="space-y-2.5">
          {todayClasses.length === 0 ? (
            <div className="py-10 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200">
              <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Bugün planlı ders yok</p>
            </div>
          ) : (
            todayClasses.map((cls, i) => {
              const isTrainer = cls.teacherId === currentUserId;
              return (
                <div key={i} className="flex items-center gap-4 p-4 bg-white border border-slate-100 rounded-2xl shadow-sm active:bg-slate-50 transition-all border-l-4 border-l-indigo-500">
                  <div className={`w-11 h-11 rounded-xl flex flex-col items-center justify-center shrink-0 ${isTrainer ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-900 border border-slate-100'}`}>
                  ...
                  <div className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-tighter ${isTrainer ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-600'}`}>
