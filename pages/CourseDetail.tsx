...
export const CourseDetail: React.FC<CourseDetailProps> = ({ courseId, onBack, currentUser, onUserClick }) => {
  ...
  const [attendanceList, setAttendanceList] = useState<Record<string, boolean>>(
    course.studentIds.reduce((acc, id) => ({ ...acc, [id]: true }), {})
  );

  const toggleAttendance = (id: string) => {
    setAttendanceList(prev => ({ ...prev, [id]: !prev[id] }));
  };
                ...
                <div key={student.id} className="flex items-center justify-between p-1.5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                   ...
                   <div className="flex gap-1.5">
                      ...
                      <button 
                        ...
                        className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${!attendanceList[student.id] ? 'bg-rose-500 text-white shadow-md' : 'bg-white dark:bg-slate-900 text-slate-300'}`}
                      >
      ...
      <div className="fixed inset-0 z-[600] bg-slate-950 flex flex-col p-6 animate-in fade-in duration-500">
        <header className="flex justify-between items-center mb-8">
           <div className="text-white">
              <h2 className="text-sm font-black uppercase tracking-widest leading-none">HIZLI QR YOKLAMA</h2>
  ...
  return (
    <div className="w-full page-transition px-4 space-y-2 pb-24 pt-1 transition-all overflow-hidden">
      <header className="flex items-center gap-2 mb-1">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[6px] font-black text-indigo-500 uppercase tracking-[0.2em] mt-0.5">DERS AYRINTISI</p>
        </div>
      </header>
      ...
      {isTeacherOfThisCourse && (
        <section className="grid grid-cols-2 gap-2 mt-1">
           <button 
             onClick={() => setViewMode('qr-attendance')}
             className="bg-indigo-600 p-2.5 rounded-2xl text-white flex flex-col items-center justify-center gap-1 shadow-lg active:scale-95 transition-all"
           >
           ...
           <button 
             onClick={() => setViewMode('manual-attendance')}
             className="bg-slate-900 dark:bg-slate-800 p-2.5 rounded-2xl text-white flex flex-col items-center justify-center gap-1 shadow-lg active:scale-95 transition-all"
           >
      ...
      <section className="grid grid-cols-2 gap-1.5">
        ...
        <div className="bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-50 dark:border-slate-800 flex flex-col gap-0.5 shadow-sm">
          ...
          <p className="text-[9px] font-bold text-slate-700 dark:text-slate-200 truncate">{course.location || 'Kampüs'}</p>
        </div>
      </section>

      {/* Instructor Card with Quick Links */}
      <div className="bg-slate-900 dark:bg-slate-800 rounded-2xl p-2.5 text-white flex items-center gap-2.5 shadow-md relative overflow-hidden">
        ...
        <div className="flex gap-1.5 px-1 border-l border-white/10">
          ...
          <button className="w-6 h-6 bg-pink-500/20 rounded-lg flex items-center justify-center text-pink-400 active:scale-90 transition-transform">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
          </button>
        </div>
      </div>
