...
export const Calendar: React.FC<CalendarProps> = ({ currentUser }) => {
  ...
  return (
    <div className="w-full page-transition px-4 space-y-6 pt-4">
      {/* Day Picker - Replaces Header */}
      <div className="flex justify-between bg-slate-50 p-1.5 rounded-[1.75rem] border border-slate-100 shadow-inner">
        {weekDays.map((dayIdx) => {
      ...
      <div className="space-y-4 pb-10">
        <div className="flex items-center justify-between px-1">
          ...
          <span className="text-[9px] font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg">{activeClasses.length} DERS</span>
        </div>
        ...
        {activeClasses.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center bg-white rounded-[2.5rem] border border-dashed border-slate-200">
            ...
            <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">Ders planlanmamış</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {activeClasses.map((cls, i) => {
              ...
              return (
                <div key={i} className="bg-white p-4 rounded-3xl border border-slate-100 flex items-center gap-4 shadow-sm active:scale-[0.98] transition-all">
                  <div className="flex flex-col items-center justify-center w-12 py-2.5 bg-slate-50 rounded-2xl border border-slate-100 shrink-0">
                    <span className="text-[10px] font-black text-slate-900 leading-none">{cls.startTime}</span>
                    <div className="w-3 h-[1px] bg-slate-200 my-1.5"></div>
